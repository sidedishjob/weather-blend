...
const getWeatherIcon = (weather: string, isBlended = false) => {
  ...
  if (weatherLower.includes("小雨") || weatherLower.includes("drizzle")) {
    return (
      <CloudDrizzle className={`${iconSize} text-blue-400 drop-shadow-lg`} />
    );
  }
  return <Cloud className={`${iconSize} text-gray-400 drop-shadow-lg`} />;
};
...
export function WeatherCard({
  ...
  return (
    ...
    >
      <CardContent
        className={`${
          isBlended ? "p-4 sm:p-6 lg:p-8" : "p-3 sm:p-4"
        } relative overflow-hidden`}
      >
        {/* 背景装飾 */}
        {isBlended && (
          <>
            <div className="absolute inset-0 bg-gradient-to-br from-blue-500/10 via-transparent to-sky-500/20 opacity-60"></div>
          ...
          <div className="flex items-center justify-between gap-2 sm:gap-4">
                ...
                <div
                  className={`${
                    isBlended
                      ...
                      : "text-sm text-blue-700/80"
                  } font-medium`}
                >
                  <div
                    className={`${
                      isBlended
                        ? "text-sm sm:text-base lg:text-xl text-blue-950"
                        : "text-xs sm:text-sm text-blue-900"
                    } font-medium`}
                  >
                    {data.weather}
                  </div>
            ...
            <div className="text-right space-y-1 sm:space-y-2 flex-shrink-0">
              ...
              <div
                className={`${
                  isBlended
                    ? "text-xs sm:text-sm text-blue-800"
            ...
            <div className="mt-2 sm:mt-3 flex items-center justify-between">
              ...
              <div className="hidden sm:flex items-center space-x-1">
                ...
                <span className="text-xs text-blue-800">
                  体感温度{" "}
                  {Math.round(data.temperature + (Math.random() - 0.5) * 4)}°C
                </span>
              </div>
            </div>
          )}
          ...
          {isBlended && data.confidence && (
            <div className="mt-4 sm:mt-6 pt-3 sm:pt-4 border-t border-blue-300/50 space-y-3 sm:space-y-4">
              ...
              <div className="flex items-center justify-between">
                <span className="text-blue-900 text-xs sm:text-sm font-medium">
                  予報信頼度
                </span>
                <div className="flex items-center space-x-1 sm:space-x-2">
                  ...
                  <span className="text-blue-950 font-semibold text-xs sm:text-sm">
